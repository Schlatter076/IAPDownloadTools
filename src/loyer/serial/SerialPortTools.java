...
public class SerialPortTools {
  ...
  public static String crc16_XMODEM(byte[] cbuf) {
    int wCRCin = 0x0000;
    for(int j = 0; j < cbuf.length; j++) {
      wCRCin ^= ((cbuf[j] & 0xFF) << 8);
      for (int i = 0; i < 8; i++)
      {
          if ((wCRCin & 0x8000) != 0)
              wCRCin = (wCRCin << 1) ^ 0x1021;
          else
              wCRCin = wCRCin << 1;
      }
    }
    return String.format("%04X", wCRCin & 0xFFFF);
  }
  
  public static byte[] crc16_XMODEM_2(byte[] cbuf) {
    int wCRCin = 0x0000;
    for(int j = 0; j < cbuf.length; j++) {
      wCRCin ^= ((cbuf[j] & 0xFF) << 8);
      for (int i = 0; i < 8; i++)
    ...
    byte[] res = new byte[2];
    res[0] = (byte) (((wCRCin & 0xFFFF) >> 8) & 0xFF);
    res[1] = (byte) ((wCRCin & 0xFFFF) & 0xFF);
    return res;
  }
  ...
  public static byte[] addCrcCheck(byte[] source) {
    ...
    dest[dest.length - 3] = (byte) ((wCrc >> 8) & 0xff);
    dest[dest.length - 2] = (byte) (wCrc & 0xff);
    dest[dest.length - 1] = source[source.length - 1];
    return dest;
  }
